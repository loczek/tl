...
resource "aws_instance" "tl_monitoring" {
  ...
  user_data = templatefile("${path.root}/../scripts/monitoring.sh.tmpl", {
    INSTALL_DOCKER_SCRIPT = file("${path.root}/../scripts/install-docker.sh")
    INSTALL_NOMAD_SCRIPT  = file("${path.root}/../scripts/install-nomad.sh")
    SETUP_NOMAD_SCRIPT    = file("${path.root}/../../nomad/monitoring.hcl")
  })
...
resource "aws_instance" "tl_database" {
  ...
  user_data_replace_on_change = true
  iam_instance_profile        = aws_iam_instance_profile.nomad_server_profile.name
  user_data = templatefile("${path.root}/../scripts/db.sh.tmpl", {
    INSTALL_DOCKER_SCRIPT = file("${path.root}/../scripts/install-docker.sh")
    INSTALL_NOMAD_SCRIPT  = file("${path.root}/../scripts/install-nomad.sh")
    SETUP_NOMAD_SCRIPT    = file("${path.root}/../../nomad/database.hcl")
  })
...
resource "aws_instance" "tl_instance" {
  ...
  user_data_replace_on_change = true
  iam_instance_profile        = aws_iam_instance_profile.nomad_server_profile.name
  user_data = templatefile("${path.root}/../scripts/ingress.sh.tmpl", {
    INSTALL_DOCKER_SCRIPT = file("${path.root}/../scripts/install-docker.sh")
    INSTALL_NOMAD_SCRIPT  = file("${path.root}/../scripts/install-nomad.sh")
    SETUP_NOMAD_SCRIPT    = file("${path.root}/../../nomad/ingress.hcl")
  })
...
resource "aws_eip_association" "static_ip_assoc" {
  ...
  allocation_id = var.public_eip_id
}
